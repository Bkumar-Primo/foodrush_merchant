...
export function parseOrdersJson(raw: string): Order[] {
  try {
...
export function parseInventoryJson(raw: string): MenuItem[] {
  try {
    const parsed: unknown = JSON.parse(raw);
    if (!Array.isArray(parsed)) return [];
    return parsed.filter(isMenuItem);
  } catch {
    return [];
  }
}
...
export type SyncMessage =
  | { type: "ORDERS_UPDATE"; data: Order[] }
  | { type: "INVENTORY_UPDATE"; data: MenuItem[] };

export function isSyncMessage(value: unknown): value is SyncMessage {
  if (!isRecord(value) || typeof value.type !== "string") return false;
  if (!Array.isArray(value.data)) return false;

  if (value.type === "ORDERS_UPDATE") {
    return value.data.every(isOrder);
  }
  if (value.type === "INVENTORY_UPDATE") {
    return value.data.every(isMenuItem);
  }
  return false;
}
